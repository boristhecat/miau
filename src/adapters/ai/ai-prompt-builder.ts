...
export function buildSystemPrompt(): string {
  return [
    ...
    "- learningContext: historical performance for this setup type in this regime. winRatePct is a percentage (e.g. 63 = 63% win rate). dominantFailureType (if present) is the most common failure mode with >= 40% share and >= 3 instances — treat as real signal. sampleSize 20-40 = weak prior; above 80 = meaningful weight. Absent means insufficient data.",
    "",
    ...
    "6. Historical performance — if learningContext is present, does the recorded pattern support or contradict the current signal? Flag contradictions explicitly; weight by sampleSize.",
    "",
    "Output rules:",
